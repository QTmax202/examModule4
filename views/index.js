...
function getCity(city) {
    return `<tr>
                <td>${city.id}</td>
                <td><a href="#" onClick="detailCity(${city.id})" style="color: black">${city.name}</a></td>
...
function showDelete(cityId) {
    ...
    document.getElementById("btn_delete_yes").innerHTML = `<button onclick="DeleteCity(${cityId})" class="btn btn-outline-secondary">Xóa</button>`
    document.getElementById("btn_home").innerHTML = `<button onclick="closeDetailForm()" class="btn btn-secondary">Back</button>`
...
function updateCity(){
    ...
    if (gdp < 0 || population < 0 || area < 0 || name === '' || description === '' || gdp === '' || population === '' || area === ''){
        alert("Số liệu sai!");
        return false;
    }
    let data = {
...
function detailCity(cityId){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            document.getElementById("btn_edit").innerHTML = `<button onclick="showEdit(${data.id},1)" class="btn btn-outline-secondary">Sửa</button>`
        }
    })
}
